...
### ☐ `addPlanarReflection`'s "SSR pass when available" is currently always unavailable
- **Where:** `src/material/planarReflection.js`
- **Why skipped:** `postfx/` (Phase 7, where an SSR pass would live) doesn't exist yet — there's no global registry of active postfx passes to auto-detect "is SSR available" from. `addPlanarReflection` instead accepts an `ssrPass` option: pass any truthy value to opt into `ReflectorForSSRPass` instead of the standalone `Reflector`; its *shape* isn't validated, since Phase 7 hasn't defined what an SSR pass object looks like yet. Every call today, with no `ssrPass` argument, uses the standalone `Reflector` fallback.
- **Revisit when:** Phase 7 builds an SSR pass and exposes it (e.g. via `graph3d.postfx.enable('ssr')`) — wire that return value as the thing callers pass to `ssrPass`, and consider validating its shape then.
