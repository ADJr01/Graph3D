...
|---|---|---|
| `core/` | Renderer, loop, registry, capabilities, workers, frame budget | Scene contents, chart logic, materials — **except `Graph3D`'s own `postfx` getter**, a second sanctioned composition-root exception alongside `GraphScene` below: `Graph3D` lazily builds a `PostFX` (`postfx/`, Prompt 116) bound to the active scene's `.three`/`.camera.three`, exactly mirroring why `Graph3D` is already allowed to import `GraphScene` — it is the one place that legitimately wires every layer together, not `core/`'s other files. **`Graph3D.devtools` / `core/GraphDevTools.js` (Prompt 178) is a fourth, dev-only sanctioned exception**: `GraphDevTools.listActiveTimelines` reads the shared `anim` singleton (`anim/GraphAnim.js`'s `timelines` getter) directly — the only introspection here that has no other public registry to read from; every other `devtools` method (`dumpSceneGraph`, `pickingDebugOverlay`, `frustumDebugOverlay`, `octreeDebugOverlay`, `selectionDebugOverlay`) takes the object to inspect as a caller-supplied argument instead of importing `scene/`/`object/`/`compose/selection` directly. This doesn't close a cycle despite `anim/GraphAnim.js` already importing `core/Graph3DLoop.js` (the same "already crosses once" situation `compose/selection`'s `SelectionTransition` exception documents below): `Graph3DLoop.js` has no import back into `anim/` or into `GraphDevTools.js`, so the two crossings don't meet (`madge --circular src/` stays clean). `Graph3D.devtools` itself throws in production (`process.env.NODE_ENV === 'production'`) so none of this reaches a real user's bundle once a consumer's own bundler dead-code-eliminates the guarded call sites. |
...
4. Have a leak test in `tests/integration/<class>-disposal.test.js` that creates and disposes the class N times (default 1000) and asserts no leak occurred. **The standard is spy/structural assertions, not `renderer.info.memory` polling** (reconciled in improvement.md initiative (d) PR 4 — this file originally described the latter, but the test suite never followed it, for good reason: these tests run under jsdom with no real WebGL context, where `renderer.info.memory` isn't meaningfully populated anyway). Concretely: `expect(scene.children.length).toBe(0)` after the loop for anything added to a scene; `vi.spyOn(x, 'dispose')` on the geometry/material/texture a class wraps, asserted `toHaveBeenCalledOnce()`, for anything that isn't scene-parented; subscription-specific assertions (`loop.isRunning === false`, no leaked event listeners) for classes whose only resource is a loop/DOM subscription. This is deterministic — no GC-timing flakiness — and every one of the 14 files under `tests/integration/*-disposal.test.js` already follows it.
...
- [ ] If a new public concept was introduced, it has a docs page or section under `site/`.
