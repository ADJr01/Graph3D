...
| `scene/` | Scene composition: cameras, lights, environment, shadows, clip planes | Chart data, instancing decisions, materials beyond environment |
| `object/` | Object/mesh wrappers, instancing, octree, loaders | Chart-type-specific logic, data binding, scales |
| `compose/` | Scales, generators, layouts, palettes, axes, annotations | Three.js objects directly (works on plain arrays/numbers) |
...
- Each layer has its own `index.js` declaring its public surface. Other layers import only from that `index.js`, never from internal files.
