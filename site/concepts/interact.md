...
### Precise picking under an actively moving camera

`Picker.camera` refreshes the camera's own `matrixWorld` on every access, for the same reason the paragraph above refreshes the scene's: a control like `OrbitControls` writes `camera.position`/`.quaternion` synchronously, straight from its own `pointermove` listener on the canvas — not through the render loop — so `camera.matrixWorld` can lag behind by up to a frame while the user is actively rotating or panning. `THREE.Raycaster.setFromCamera()` and `Vector3.project()`/`.unproject()` all read `matrixWorld` directly with no refresh of their own, so without this, a hover/click/drag fired mid-gesture would ray-test against wherever the camera *was* a frame ago, not where it visually is — imprecise picks that get worse the faster the camera is moving, regardless of the chart's own position or orientation. `PointerRouter` reads the camera exclusively through this getter (hover, click, drag-unproject, and label-click projection all inherit the fix for free); `Brush`/`Lasso` hold their own camera reference and get the matching fix directly in `regionSelect.js#matchedIndicesForChart`, alongside its existing scene refresh.
...
- **`effects.list()`** — every preset's name and option schema, for site/tooling to enumerate.
...
- **No headless-GL rendering in this project's test suite.** Every shader effect (`material.effects`), the default hover/select visuals, and every pointer/pick interaction are tested structurally (state, uniforms, mesh/material identity, disposal) against `jsdom`, never actually compiled/rendered against a real GL context — a syntax error inside an untested shader branch wouldn't surface until a real browser render (the same limitation `site/concepts/material.md`'s own Phase 6 notes flag).
